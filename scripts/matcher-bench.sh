...
if [ "$REFRESH" = "1" ]; then
	if [ "$STATUS" -ne 0 ]; then
		...
		if fetch "$DUMP" "$STAGE/corpus-replay.json.gz" &&
			...
			echo ""
			echo "       EXPECT THAT TO FAIL. A refresh is not one command."
			echo "       pipeline_floors.json carries a 'measured' block and a 'commit'"
			echo "       that are pinned to the fixture, so re-recording moves both and"
			echo "       two tests say so. The failure prints every value to write:"
			echo "       copy them into pipeline_floors.json and re-run."
			echo ""
			echo "       The three FLOORS (min_recall, min_clean, max_false_verify_rate)"
			echo "       are rates, so a bigger corpus at the same quality does not move"
			echo "       them. Those you only touch on purpose: if the new recording is"
