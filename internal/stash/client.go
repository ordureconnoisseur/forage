...
const findOwnedAttributionQuery = `
query ForagerOwnedAttribution($page: Int!, $perPage: Int!) {
  findScenes(filter: { page: $page, per_page: $perPage, sort: "id", direction: ASC }) {
    scenes {
      stash_ids { endpoint stash_id }
      studio { stash_ids { endpoint stash_id } }
      performers { stash_ids { endpoint stash_id } }
    }
  }
}`

// OwnedSceneAttribution is one local scene's StashDB cross-id and the cross-ids
// of its studio + performers. Studio cross-id is "" when the scene's studio has
// no StashDB mapping (or no studio).
type OwnedSceneAttribution struct {
	SceneStashDBID      string
	StudioStashDBID     string
	PerformerStashDBIDs []string
}

// FindOwnedSceneAttribution sweeps the whole library once and returns, per scene
// that carries a StashDB cross-id, its studio + performer cross-ids. Scenes
// without a scene-level cross-id are skipped (they can't be attributed to a
// StashDB-keyed subject). One sweep; no StashDB calls.
func (c *Client) FindOwnedSceneAttribution(ctx context.Context) ([]OwnedSceneAttribution, error) {
	...
	var out []OwnedSceneAttribution
	err := pagedQuery(ctx, c, "findScenes attribution", findOwnedAttributionQuery, nil, 1000, func(r resp) (int, bool) {
		for _, s := range r.FindScenes.Scenes {
			sid := PickStashDBID(s.StashIDs)
			if sid == "" {
				continue
			}
			a := OwnedSceneAttribution{SceneStashDBID: sid}
			if s.Studio != nil {
				a.StudioStashDBID = PickStashDBID(s.Studio.StashIDs)
			}
			for _, p := range s.Performers {
				if pid := PickStashDBID(p.StashIDs); pid != "" {
					a.PerformerStashDBIDs = append(a.PerformerStashDBIDs, pid)
				}
			}
			out = append(out, a)
		}
