...
const (
	...
	// (many pages each), so fewer concurrent studios keeps total in-flight
	// request volume polite to StashDB.
...
func RefreshStudios(ctx context.Context, sc *stash.Client, sdb *stashdb.Client, db *sql.DB, log *slog.Logger) error {
	...
	// last_release) are owned by the count sync (cache.SyncStashDBCounts) and
	// deliberately left out of the SET list so this pass never clobbers them; a
	// fresh INSERT gets their column defaults (0) until the count pass fills them.
