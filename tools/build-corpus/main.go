...
func main() {
	...
	if qbitC != nil {
		fmt.Fprintf(os.Stderr, "listing qBit torrents…\n")
	...
	if sabC != nil {
		fmt.Fprintf(os.Stderr, "pulling SAB history…\n")
	...
	for _, c := range candidates {
		...
		if seenScenes[scene.StashDBID] {
			s.dupScene++
			continue
		}
		seenScenes[scene.StashDBID] = true

		entries = append(entries, entry{
			ID:            slugify(c.Release),
			Release:       c.Release,
			ExpectedScene: scene.StashDBID,
			Source:        c.Source,
			FilePath:      c.FilePath,
		})
		if *verbose {
			fmt.Fprintf(os.Stderr, "  ✓ %s → %s\n", trim(c.Release, 70), shortID(scene.StashDBID))
		}
	}
...
func writeYAML(f *os.File, entries []entry) {
	...
	fmt.Fprintln(f, "# Auto-generated from qBit + SAB history by tools/build-corpus.")
	...
	fmt.Fprintln(f, "# pair derived from a real grab. The matcher should return")
	fmt.Fprintln(f, "# expected_scene_id at rank 1 when fed the `release` string.")
	fmt.Fprintln(f)
